...
int main(int argc, char *argv[]) {
  if (argc < 2 || argc > 7) err(1, "incorrect argument count\n argc: %d", argc);
  int8_t opt;
  ...
  while ( (opt = getopt(argc, argv, "N:l")) != ERR){
    switch(opt){
      case 'N':
        ++Nflag;
        if (optarg) thread_count = atoi(optarg);
      case 'l':
        ++lflag;
      default :
        break;
  ...
  if (thread_count < 1) err(1, "Cannot run with less than 1 thread\n");
  printf("argument count:%d\n", argc);
  if (Nflag) printf("thread count detected\tthreads: %d\n", thread_count);
  if (lflag) printf("logging detected\n");
  printf("optind:%d\n", optind);
  if (optind+2 == argc) {
  ...
  //socket_setup(argv[optind], argv[optind+1]);
  return 0;
}